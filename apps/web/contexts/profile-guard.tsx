...
export function ProfileGuardProvider({ children }: { children: ReactNode }) {
  ...
  const [modalKind, setModalKind] = useState<'signin' | 'complete' | null>(null)
  ...
  const guardAction = useCallback((action: () => void) => {
    if (signedIn === false) {
      ...
      setModalKind('complete')
    } else {
      action()
    }
  }, [signedIn, profileCompleted])
  ...
  return (
    <Ctx.Provider value={{ profileCompleted, signedIn, refreshStatus, guardAction }}>
      {children}
            ...
            {modalKind === 'signin' ? (
              <>
                ...
                <p className="text-center text-sm text-gray-500 mb-6 leading-relaxed">
                  You need an account to send interests, chat, or connect with matches.
                  Sign in or create one to continue.
                </p>
